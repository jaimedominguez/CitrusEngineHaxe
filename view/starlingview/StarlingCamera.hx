...
/**
	 ...
	 */
class StarlingCamera extends ACitrusCamera
{
    
    public function new(viewRoot : starling.display.Sprite)
    {
        super(viewRoot);
    }
    ...
    override private function initialize() : Void {
        ...
        cameraLensWidth = (try cast(_ce, StarlingCitrusEngine) catch(e:Dynamic) null).starling.stage.stageWidth;
        cameraLensHeight = (try cast(_ce, StarlingCitrusEngine) catch(e:Dynamic) null).starling.stage.stageHeight;
        ...
        _m = (try cast(_viewRoot, starling.display.Sprite) catch(e:Dynamic) null).transformationMatrix;
    }
    
    /**
		 * @inheritDoc
		 */
    override private function onResize(w : Float, h : Float) : Void {
        cameraLensWidth = (try cast(_ce, StarlingCitrusEngine) catch(e:Dynamic) null).starling.stage.stageWidth;
        cameraLensHeight = (try cast(_ce, StarlingCitrusEngine) catch(e:Dynamic) null).starling.stage.stageHeight;
    }
    ...
    /**
		 ...
		 * @param	factor
		 */
    override public function zoom(factor : Float) : Void {
        if (_allowZoom)
        {
            _zoom *= factor;
        }
    ...
    /**
		 * @inheritDoc
		 */
    override public function zoomFit(width : Float, height : Float, storeInBaseZoom : Bool = false) : Float
    {
        if (_allowZoom)
        {
            var ratio : Float;
    ...
    /**
		 ...
		 */
    override public function rotate(angle : Float) : Void {
        if (_allowRotation)
        {
            _rotation += angle;
        }
    ...
    /**
		 ...
		 * @param	angle in radians.
		 */
    override public function setRotation(angle : Float) : Void {
        if (_allowRotation)
        {
            _rotation = angle;
        }
    ...
    /**
		 ...
		 * @param	factor
		 */
    override public function setZoom(factor : Float) : Void {
        if (_allowZoom)
        {
            _zoom = factor;
        }
    ...
    /**
		 * @inheritDoc
		 */
    override public function getZoom() : Float
    {
        return _zoom;
    }
    ...
    override public function update() : Void {
        ...
        if (_allowZoom)
        {
            ...
            if (bounds!=null && (boundsMode == ACitrusCamera.BOUNDS_MODE_AABB || boundsMode == ACitrusCamera.BOUNDS_MODE_ADVANCED)){
                var lwratio : Float = (_aabbData.rect.width * _camProxy.scale) / bounds.width;
                var lhratio : Float = (_aabbData.rect.height * _camProxy.scale) / bounds.height;
        ...
        if (bounds!=null)
        {
            if (boundsMode == ACitrusCamera.BOUNDS_MODE_AABB){
            ...
            else if (boundsMode == ACitrusCamera.BOUNDS_MODE_OFFSET){
            ...
            else if (boundsMode == ACitrusCamera.BOUNDS_MODE_ADVANCED)
        ...
        if (parallaxMode == ACitrusCamera.PARALLAX_MODE_TOPLEFT)
        {
            _m.identity();
        ...
        (try cast(_viewRoot, starling.display.Sprite) catch(e:Dynamic) null).transformationMatrix = _m;
    }
    ...
    public function renderDebug(sprite : flash.display.Sprite) : Void {
        ...
        var AABB : Dynamic = MathUtils.createAABBData(
        ...
        var c : Float = Math.cos(_camProxy.rotation);
        var s : Float = Math.sin(_camProxy.rotation);
        ...
        if (bounds!=null && !bounds.containsRect(AABB.rect)){
    ...
    /**
		 ...
		 */
    public function pointFromLocal(x : Float, y : Float, resultPoint : Point = null) : Point
    {
        _p.setTo(x, y);
        return (try cast(_viewRoot, starling.display.Sprite) catch(e:Dynamic) null).globalToLocal(_p, resultPoint);
    }
    ...
    {
        return (try cast(_viewRoot, starling.display.Sprite) catch(e:Dynamic) null).localToGlobal(p);
    }
    
    /**
		 * @inheritDoc
		 */
    override public function get_allowZoom() : Bool {
        return _allowZoom;
    }
    ...
    /**
		 * @inheritDoc
		 */
    override public function get_allowRotation() : Bool {
    ...
    /**
		 * @inheritDoc
		 */
    override public function set_allowZoom(value : Bool) : Bool {
        if (!value)
        {
            _zoom = 1;
            _camProxy.scale = 1;
    ...
    /**
		 * @inheritDoc
		 */
    override public function set_allowRotation(value : Bool) : Bool {
        if (!value)
        {
            _rotation = 0;
