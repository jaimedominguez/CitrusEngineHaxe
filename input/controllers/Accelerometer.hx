...
class Accelerometer extends InputController
{
    public var acceleration(get, never) : Vector3D;
    public var rotation(get, never) : Vector3D;

    private var _accel : flash.sensors.Accelerometer;
    
    //current accel
    private var _a : Vector3D = new Vector3D();
    //target accel
    private var _t : Vector3D = new Vector3D();
    
    //rotation
    private var _rot : Vector3D = new Vector3D();
    //previous rotation
    private var _prevRot : Vector3D = new Vector3D();
    
    //only start calculating when received first events from device.
    private var receivedFirstAccelUpdate : Bool = false;
    ...
    /**
		 ...
		 */
    private function customActions() : Void {
        if (actions == null)
        {
            actions = new Dictionary();
            Reflect.setField(actions, "left", false);
            Reflect.setField(actions, "right", false);
            Reflect.setField(actions, "down", false);
            Reflect.setField(actions, "up", false);
            Reflect.setField(actions, "jump", false);
        }
   ...
   public function get_acceleration() : Vector3D
    ...
    /*
		 * Rotation Vector
		 */
   public function get_rotation() : Vector3D
    {
        return _rot;
    }
}
